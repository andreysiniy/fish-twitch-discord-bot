...
@pytest.mark.asyncio
async def test_fishevent_ignores_unknown_optional_duration_token(monkeypatch) -> None:
    from commands.admin import AdminCog
    ...
    monkeypatch.setattr("commands.admin.get_channel_id", channel_id)
    await AdminCog.fishevent._callback(AdminCog(bot), ctx, "1", "duration_seconds")
